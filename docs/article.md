...
Before the enhancement, MockNest Serverless used a buffered Lambda response. That means the full response had to be built before it was returned to API Gateway. API Gateway has a 10 MB payload limit for non-WebSocket APIs [2], but in the Lambda payload is limited to 6 MB for both request and response [3].

Although for many endpoints, 6 MB is enough, for large CSV exports, PDFs, generated files, or slow streamed responses, it is not. So I added response streaming to a Kotlin Lambda running on the JVM.
...
For response streaming, I refactored the handler to `RequestStreamHandler`:
...
In my case, I added a small parser that converts the raw API Gateway event into domain-level HTTP request object. That kept the rest of the application code pretty much unchanged.
